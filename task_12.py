...
ORGANISATIONS_SEARCH_API_SERVER = "https://search-maps.yandex.ru/v1/"
GEOCODER_API_KEY = "40d1649f-0493-4b70-98ba-98533de7710b"
ORGANISATIONS_SEARCH_API_KEY = "dda3ddba-c9ea-4ead-9010-f43fbc15c6e3"
...
class App(QMainWindow):
    ...
    def unitUI(self):
        ...
        self.latitude_input.setPlaceholderText('Широта')
        ...
        self.longitude_input.setPlaceholderText('Долгота')
    ...
    def organizations_search(self, latitude, longitude):
        address_ll = f"{latitude},{longitude}"
        search_params = {
            "apikey": ORGANISATIONS_SEARCH_API_KEY,
            "lang": "ru_RU",
            "text": address_ll,
            "ll": address_ll,
            "results": "1",
        ...
        response = requests.get(ORGANISATIONS_SEARCH_API_SERVER, params=search_params)
        if not response:
            return
        ...
        try:
            ...
            org_longitude, org_latitude = organization["geometry"]["coordinates"]
        except IndexError or KeyError:
            return
        delta_long = abs(float(org_longitude) - float(longitude))
        delta_lat = abs(float(latitude) - float(org_latitude))
        delta_x = delta_long * 111111 * cos((float(org_latitude) + float(latitude)) / 2)
        delta_y = delta_lat * 111111
        radius = sqrt(delta_x ** 2 + delta_y ** 2)
        if radius <= 50:
            self.search_result_label.setText(f"{org_name}. Адрес: {org_address}")
            return org_longitude, org_latitude
    ...
    def makepoint(self):
        ...
        try:
            ...
            self.longitude_input.setText(toponym_latitude)
            self.latitude_input.setText(toponym_longitude)
    ...
    def changecoords(self, *point_mode):
        if not point_mode:
            point_mode = False
        try:
            self.longitude = str(float(self.longitude_input.text()))
            self.latitude = str(float(self.latitude_input.text()))
    ...
    def mousePressEvent(self, event, left):
        x, y = event.x(), event.y()
        longitude = str(float(self.longitude) + ((x - MAP_WIDTH / 2) * (1.45 / 2 ** self.z)))
        latitude = str(float(self.latitude) - ((y - MAP_HEIGHT / 2) * (1.2 / 2 ** self.z)))
        if left:
            self.point = f"{longitude},{latitude},pmgnm1"
            self.geocoder(f"{longitude},{latitude}")
        else:
            try:
                longitude, latitude = self.organizations_search(latitude, longitude)
                self.point = f"{longitude},{latitude},pmgnm1"
            except TypeError:
                self.search_result_label.setText('Организация не найдена')
